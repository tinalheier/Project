...
def compute_skyplot_terrain(data_dict, observation_time, day, year, maskElevation, active_GNSS):
    ...
    def unpack(system_list):
        ...
        for satname, x, y, z, bearing, elevation in system_list:
            sat.append(satname)
            az.append(bearing)
...
def compute_skyplot_data(day, year, observation_time, receiverCartesianPos,maskElevation):
    ...
    sat_GPS, az_GPS_deg, zen_GPS = unpack_results(results_GPS)
    sat_Galileo, az_Galileo_deg, zen_Galileo = unpack_results(results_Galileo)
    sat_Beidou, az_Beidou_deg, zen_Beidou = unpack_results(results_Beidou)
    sat_Glonass, az_Glonass_deg, zen_Glonass = unpack_results(results_Glonass)
...
def plot_skyplot(data):
    ...
    def plot_system(system_data, label, color):
        ...
        for i, sat in enumerate(system_data["sat"]):
            ax.text(az[i], r[i], sat, fontsize=6)
            print(sat, az[i], r[i])
    ...
    ax.set_yticklabels(["90°", "60°", "30°", "0°"])  # invertert

    ax.set_title(f"Skyplot\nDate: {data['date']} Time: {data['time']} position: {data['pos']}")
